...
// Old Sprite Struct Defintions 
#include "LbSprite.h"

// Library 8.1 names — function pointers set by the game to route into library
extern TbError(*BfDraw_Sprite)(long x, long y, const TbSprite *spr);
extern TbError(*BfDraw_OneColourSprite)(long x, long y, const TbSprite *spr, const TbColour colour);
extern TbError(*BfDraw_ScaledSprite)(long xpos, long ypos, const TbSprite *sprite, long dest_width, long dest_height);
extern TbError(*BfDraw_SpriteUsingScalingData)(long posx, long posy, const TbSprite * sprite);

struct bgui_window;
extern bgui_window * ImDraw_draw_getWindows();

/**
 * @brief Holds the clipped pixel boundaries of a sprite after viewport range checking.
 */
struct DrawRetParams
{
	short left;   ///< First visible column (from left edge of sprite).
	short right;  ///< One-past-last visible column.
	short top;    ///< First visible row (from top edge of sprite).
	short btm;    ///< One-past-last visible row.
};
...
extern "C" {
#endif
	namespace BullfrogLib
	{
#define SPRITE_FORMAT_V2
...
#pragma pack()
		 ...
		 */
		TbError LbSpriteSetup(TbSprite *start, const TbSprite *end, const unsigned char * data);

		/** Set up data pointers in all arrays of sprites from given setup list.
		 */
		TbError LbSpriteSetupAll(TbSetupSprite t_setup[]);

		/** Clear data pointers in all arrays of sprites from given setup list.
		 */
		TbError LbSpriteClearAll(TbSetupSprite t_setup[]);
		 ...
		 */
		TbError LbSpriteReset(TbSprite *start, const TbSprite *end, const unsigned char *data);

		TbError LbSpriteResetAll();

		extern UBYTE * lbSpriteReMapPtr;

		TbError LbSpriteDrawOneColour(long x, long y, const TbSprite *spr, const TbColour colour);
		TbError LbSpriteDraw(long x, long y, const TbSprite *spr);
		TbError LbSpriteDrawRemap(long x, long y, const TbSprite *spr, const UBYTE *cmap);
		 ...
		 */
		TbError LbSpriteDrawUsingScalingData(long posx, long posy, const TbSprite *sprite);
		 ...
		 */
		TbError DrawAlphaSpriteUsingScalingData(long posx, long posy, const TbSprite *sprite);
		 ...
		 * @see LbSpriteSetScalingData()
		 */
		TbError DrawSpriteWthShadowUsingScalingData(long posx, long posy, const TbSprite *sprite);

		void SetAlphaScalingData(long x, long y, long swidth, long sheight,
			long dwidth, long dheight);

		TbError LbSpriteDrawScaled(long xpos, long ypos, const TbSprite *sprite,
...
#define LbSpriteDrawResized(xpos, ypos, un_per_px, sprite) \
  LbSpriteDrawScaled(xpos, ypos, sprite, \
    ...
    ((sprite)->SHeight * un_per_px + 8) / 16)
		 ...
		 * @return
		 */
		TbError LbSpriteDrawScaledOneColour(long xpos, long ypos, const TbSprite *sprite,
			long dest_width, long dest_height, const TbColour colour);
...
#define LbSpriteDrawResizedOneColour(xpos, ypos, un_per_px, sprite, colour) \
  LbSpriteDrawScaledOneColour(xpos, ypos, sprite, \
    ...
    ((sprite)->SHeight * un_per_px + 8) / 16, colour)
		 ...
		 * @return
		 */
		TbError LbSpriteDrawScaledRemap(long xpos, long ypos, const TbSprite *sprite,
